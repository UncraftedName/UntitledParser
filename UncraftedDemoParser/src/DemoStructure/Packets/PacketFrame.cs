...
namespace UncraftedDemoParser.DemoStructure.Packets {
	...
	public class PacketFrame : DemoComponent {
		...
		// in the demo the tick is stored as part of the packetframe, 
		...
		public PacketFrame(byte[] data, ref int pointer, SourceDemo demoRef) : base(data, demoRef) {
			...
			if (Type == PacketType.Stop) {
				// tick is assumed to be the same as the previous packet
				DemoPacket = new Stop(demoRef, demoRef.Frames.Last().DemoPacket.Tick);
				pointer = data.Length; // put the point straight at the end
			} else {
		...
		public override void UpdateBytes() {
			...
			bfw = new BitFieldWriter(5 + (AlignmentByte.HasValue ? 1 : 0) + DemoPacket.Bytes.Length);
